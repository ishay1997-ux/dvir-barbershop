...
export default function AdminLoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // AuthContext will automatically verify role from Firestore
    } catch (err: any) {
      ...
      <div className="min-h-screen bg-[#1C1C1C] flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden" dir="rtl">
        <div className="w-full max-w-md text-center space-y-4 bg-[#2A2A2A] border border-red-500/30 rounded-3xl p-8 shadow-2xl">
          ...
          <button
            ...
            className="mt-4 w-full py-3 rounded-xl bg-[#C9A84C] hover:bg-[#DFCA85] text-black font-black text-xs transition-colors cursor-pointer"
          >
            התנתק ונסה עם חשבון Google אחר
          </button>
        </div>
      </div>
    );
