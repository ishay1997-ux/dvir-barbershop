...
> **גרסה:** 3.2 · **מטרה:** תוספת נגישות למשתמש (תפריט התאמות). הרכיב **אינו** הופך אתר לתואם WCAG / ת"י 5568 בעצמו — הנגישות של האתר המארח (תוויות, ניגודיות, מיקוד, alt, RTL) חייבת להיות נכונה ומאומתת בכלי כמו axe. · **מחבר:** The Cut Platform
> קובץ זה מרכז את כל ההנחיות, הדרישות, הטיפים והתבניות המשפטיות להטמעה מהירה של רכיב הנגישות בכל אתר או אפליקציית ווב עתידית.

---

## 🌟 סקירה כללית
רכיב הנגישות הוא **מערכת מודולרית שמועתקת כתיקייה אחת, ללא תלות ב-Tailwind של האתר המארח**: העיצוב שלו מקומפל לקובץ `accessibility.widget.css` עם קידומת `acc:` (כ-48KB, כ-8KB דחוס) ומגיע עם בסיס מוגבל לרכיב. נבדק ב-Next.js + Tailwind, ב-Vite ריק, ובאתר עם CSS גלובלי אגרסיבי. אין בו ייבוא של `next/*`.
...
### שלב 2: התקנת שתי תלויות (Dependencies)
...
1. **14 כלי נגישות מרכזיים בגריד משבצות אינטראקטיבי:**
   ...
   * 🖼️ **הדגשת תמונות:** מסגרת מודגשת סביב כל תמונה (הצגת טקסט `alt` — מתוכננת לשלב הבא).
...
6. **קיצורי מקלדת בינלאומיים מובנים:**
   ...
   * `Escape` – סוגר את השכבה העליונה בלבד (מודאל → תפריט), עובד מכל אלמנט. קיצורי הפתיחה מתעלמים בזמן הקלדה בשדה טקסט של האתר.
   * **הגדלת גופן:** מגדילה את גופן השורש (טקסט וריווחים ב-rem). טקסט בגודל קבוע בפיקסלים לא יוגדל.
   * `Enter` בראש הדף – קפיצה ישירה לתוכן המרכזי באמצעות `SkipToContent`.

---

## 🔧 תחזוקה: אחרי שינוי עיצוב ברכיב
* העיצוב של הרכיב מקומפל מראש. **אחרי כל שינוי ב-`className` באחד מקבצי התיקייה, או ב-`tailwind.a11y.css`, להריץ:**
  ```bash
  npm run build:a11y-css
  ```
  הפקודה מחדשת את `accessibility.widget.css` (קובץ מיוצר, לא לערוך ידנית). הטסט `src/__tests__/a11y-widget-class-prefix.test.ts` נכשל אם שכחתם, או אם מחלקה נכתבה בלי הקידומת `acc:`.
* סדר הייבוא של שלושת קובצי ה-CSS קבוע (utilities → base → host rules); ההסבר בראש `accessibility.base.css`.
* גבול כנה: מחלקות הרכיב הן `!important` בספציפיות של מחלקה אחת. כלל של האתר המארח שהוא גם `!important` וגם ספציפי יותר עדיין ינצח.
...
3. `LEGAL_TEMPLATES/accessibility-template.md` (בתיקיית הפרויקט העליונה) – תבנית הצהרת נגישות. **שימו לב:** ת"י 5568-1 (2023) זהה ל-WCAG 2.0; בנייה ובדיקה לפי WCAG 2.2 AA מכסה אותו.
