...
## 🌟 סקירה כללית
רכיב הנגישות הוא **מערכת מודולרית שמועתקת כתיקייה אחת**. הוא אינו "standalone" לחלוטין: ה-UI שלו בנוי ממחלקות **Tailwind CSS v4**, ולכן האתר המארח חייב Tailwind v4 (גרסה בלי Tailwind מתוכננת). אין בו ייבוא של `next/*`, כך שהוא עובד גם ב-Vite.
...
### שלב 2: התקנת תלויות (Dependencies) — ו-Tailwind CSS v4 באתר המארח
...
6. **קיצורי מקלדת בינלאומיים מובנים:**
   ...
   * `Escape` – סגירת כל המודאלים ותפריט הנגישות (עובד מכל אלמנט, גם מתוך שדה טקסט). קיצורי הפתיחה מתעלמים בזמן הקלדה בשדה טקסט של האתר.
